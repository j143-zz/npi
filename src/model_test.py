...
next_end_reshaped = mx.sym.Reshape(next_end, shape=(-1, 1))
square_loss = mx.sym.MakeLoss(mx.sym.square(next_end_reshaped - f_end_act1))
f_end = mx.sym.Group([mx.sym.BlockGrad(f_end_act1), square_loss])
...
f_prog = mx.sym.SoftmaxOutput(data=f_prog_dot, label=next_prog_reshaped)

# f_args
next_args_reshaped = mx.sym.Reshape(next_args, shape=(-1, MAX_ARGS_NUM, ARG_DEPTH))
...
f_args = mx.sym.Group([f_arg0_softmax, f_arg1_softmax, f_arg2_softmax])
out = mx.sym.Group([f_end, f_prog, f_args])

out_shape = out.infer_shape(curr_env=(10, 80, 4, 9 ,10), curr_prog=(10, 80),curr_args=(10, 80, 3, 10), next_end=(10, 80), next_prog=(10, 80), next_args=(10, 80, 3, 10))

out_outputs = out.list_outputs()
print out_outputs
print out_shape
